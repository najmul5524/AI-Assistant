...
def create_market_range_chart(
    ...
    """
    Creates a high-definition (1030x560 px) institutional multi-asset price range,
    ...
    font_asset = get_best_font(19, bold=True)
    font_badge = get_best_font(15, bold=True)
    font_levels = get_best_font(17, bold=False)
    font_meta = get_best_font(15, bold=False)

    # Header title
    draw.text((25, 24), chart_title, fill="#ffffff", font=font_title)
    now_str = datetime.datetime.now().strftime("%d %b %Y | Asia/Dhaka")
    draw.text((width - 250, 26), now_str, fill="#94a3b8", font=font_meta)
    ...
    for a in assets:
        ...
        # Direction Badge
        draw.rounded_rectangle([250, y + 4, 370, y + 34], radius=6, fill=a["color"])
        draw.text((268, y + 9), a["bias"], fill="#ffffff", font=font_badge)

        # Range Bar (Background)
        bar_x1 = 395
        bar_x2 = 720
        draw.rounded_rectangle([bar_x1, y + 10, bar_x2, y + 28], radius=8, fill="#e2e8f0")

        # Range Bar (Fill)
        pct = max(0.1, min(0.95, a.get("pct", 0.5)))
        fill_width = int((bar_x2 - bar_x1) * pct)
        draw.rounded_rectangle([bar_x1, y + 10, bar_x1 + fill_width, y + 28], radius=8, fill=a["color"])

        # Pivot point marker
        pivot_x = bar_x1 + fill_width
        draw.line([pivot_x, y + 4, pivot_x, y + 34], fill="#0f172a", width=3)
        ...
        # Separator line
        draw.line([20, y + 48, width - 20, y + 48], fill="#f1f5f9", width=1)
        y += 58

    # Currency Strength Footer
    draw.rounded_rectangle([15, height - 55, width - 15, height - 12], radius=8, fill="#f1f5f9")
    draw.text((25, height - 42), "Currency Strength Index:", fill="#334155", font=font_meta)
    strengths = [
        ("USD", "Bullish", "#10b981"),
        ("GBP", "Strong", "#10b981"),
        ("EUR", "Weak", "#ef4444"),
